...
ALTER TABLE sample.users ADD COLUMN birthday_at DATE; 
ALTER TABLE sample.users ADD COLUMN created_at DATETIME;
ALTER TABLE sample.users ADD COLUMN updated_at DATETIME; 
...
-- 4. (2.) 2.	В таблице products есть два текстовых поля: name с названием товара и description с его описанием. 
...
CREATE TRIGGER no_two_NULL BEFORE INSERT ON products 
FOR EACH ROW BEGIN
	IF NEW.name IS NULL AND NEW.desription IS NULL THEN
		SIGNAL SQLSTATE '45000'
		SET message_text = 'И name, и description NULL';
	END IF;
END//

INSERT products (name, desription, price, catalog_id)
VALUES (NULL, NULL, 18200.00, 1)//