...
-- ДР6
-- Пусть задан некоторый пользователь. Из всех пользователей соц. сети найдите человека, который больше всех общался с выбранным пользователем (написал ему сообщений) - Elmira Bayer:

SELECT from_user_id, CONCAT(u.firstname, ' ', u.lastname) as user_name,	COUNT(*) AS 'count_of_messages'
FROM messages m JOIN users u ON u.id = m.from_user_id WHERE to_user_id = 1
GROUP BY from_user_id ORDER BY count(*) DESC LIMIT 1;

-- Подсчитать общее количество лайков, которые получили пользователи младше 10 лет - 5:

SELECT COUNT(*) AS 'count_of_likes' FROM posts_likes WHERE post_id IN 
	(SELECT id FROM media WHERE user_id IN 
		(SELECT user_id FROM profiles WHERE YEAR(CURDATE()) - YEAR(birthday) < 10));
		
-- Определить кто больше поставил лайков (всего): мужчины или женщины - x:
	
SELECT gender, COUNT(*) AS 'count_of_likes' FROM
	(SELECT user_id AS USER, 
		(SELECT gender FROM vk.profiles WHERE user_id = user) AS gender FROM posts_likes) 
			AS zero GROUP BY gender;