...
-- ДР8
-- 1. Пусть задан некоторый пользователь. Из всех пользователей соц. сети найдите человека, который больше всех общался с выбранным пользователем (написал ему сообщений) - Elmira Bayer:

SELECT from_user_id, CONCAT(u.firstname, ' ', u.lastname) as 'человек',	COUNT(*) AS 'количество сообщений'
FROM messages m JOIN users u ON u.id = m.from_user_id WHERE to_user_id = 1
GROUP BY from_user_id ORDER BY count(*) DESC LIMIT 1;

-- 2. Подсчитать общее количество лайков, которые получили пользователи младше 10 лет - 5:

SELECT count(*) AS 'количество лайков'
FROM posts_likes pl
JOIN media m ON pl.post_id = m.id
JOIN profiles p ON p.user_id = m.user_id
WHERE  YEAR(CURDATE()) - YEAR(birthday) < 10;

-- 3. Определить кто больше поставил лайков (всего): мужчины или женщины - x:
	
SELECT gender AS 'пол', COUNT(gender) AS 'количество лайков' 
FROM profiles
JOIN posts_likes ON posts_likes.user_id = profiles.user_id
GROUP BY gender
ORDER BY COUNT(posts_likes.user_id) DESC
LIMIT 1;